        ...
        <div class="main-content-wrap">
            ...
            <div class="wg-box">
                <div class="flex items-center justify-between gap10 flex-wrap">
                    ...
                    <Link class="tf-button style-1 w208" :href="`/admin/product/save?id=${0}`"
                        ><i class="icon-plus"></i>
                        Add new</Link >
                </div>
                <div class="wg-table table-all-user">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered">
                            ...
                            <tbody>
                                <tr v-for="(product,index) in page.props.products" :key="index">
                                    <td>{{ index+1 }}</td>
                                    <td class="pname">
                                        <div class="image">
                                            <img
                                    ...
                                    <td>{{ product.name }}</td>
                                    <td>
                                        <Link :href="`/shop/product/${product.slug}`" target="_blank">{{ product.slug }}</Link>
                                    </td>
                                    ...
                                    <td>{{ product.flash_sales }}</td>
                                    <td>{{ product.featured }}</td>
                                    <td>{{ product.popular }}</td>
                                    <td>{{ product.quantity }}</td>
                                    <td>
                                        <div class="list-icon-function">
                                            <Link :href="`/admin/product/save?id=${product.id}`">
                                                <div class="item edit">
